...
if(linklesson&&lesson&&closelesson) {
    ...
    linklesson.addEventListener("click",function(a){
    	a.preventDefault(),lesson.classList.add("modal-content-show"),login.focus(),storage?(login.value=storage,email.focus()):login.focus()})
    ,closelesson.addEventListener("click",function(a){
    	a.preventDefault(),lesson.classList.remove("modal-content-show"),lesson.classList.remove("modal-error")})
    ,form.addEventListener("submit",function(a){
    	login.value&&email.value&&textletter.value?(localStorage.setItem("login",login.value),localStorage.setItem("email",email.value)):(a.preventDefault(),lesson.classList.add("modal-error"))}),
